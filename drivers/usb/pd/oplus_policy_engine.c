...
#include "../../oneplus/power/oplus_chg/oplus_warp.h"
#endif
...
#ifdef VENDOR_EDIT
/*Gang.Yan@BSP.CHG.BASIC, 2020/09/09,add for PD+SWARP adapter*/
...
static inline void start_usb_peripheral(struct usbpd *pd)
{
	...
	union extcon_property_value val;
#ifdef VENDOR_EDIT
//zhouhaikang@BSP.CHG.Basic,2020/11/19,add for chg
...
static void log_decoded_request(struct usbpd *pd, u32 rdo)
{
	...
	case PD_SRC_PDO_TYPE_BATTERY:
		usbpd_dbg(&pd->dev, "Request Battery PDO:%d Op-Pow:%dmW Pow:%dmW\n",
				pos,
...
#ifndef VENDOR_EDIT
/* tongfeng.Huang@BSP.CHG.Basic, 2020/02/13,  add for  pd+warp adapter compatibility */
		//pd_phy_update_frame_filter(FRAME_FILTER_EN_SOP |
...
static void handle_state_unknown(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	if (pd->current_pr == PR_SINK) {
		...
		if (pd->typec_mode == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER) {
			usbpd_err(&pd->dev, "notify audio acc to framework\n");

			if (!pd->partner) {
				memset(&pd->partner_identity, 0, sizeof(pd->partner_identity));
				usbpd_err(&pd->dev, "register pd partner\n");
				pd->partner_desc.usb_pd = false;
...
static void handle_snk_ready_tx(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	if (pd->send_get_src_cap_ext) {
		...
		ret = pd_send_msg(pd, MSG_GET_SOURCE_CAP_EXTENDED, NULL,
			0, SOP_MSG);
#ifdef OEM_TARGET_PRODUCT_EBBA
//this function is used by hvdcp_opti, we should not reset pd for it when driver is doing something
		if (ret && ret != -EBUSY) {
#else
		if (ret) {
#endif
...
static void usbpd_sm(struct work_struct *w)
{
	...
	if (pd->current_pr == PR_NONE) {
		...
		if (pd->current_state == PE_UNKNOWN &&
				pd->current_dr == DR_NONE && (pd->partner == NULL))
#else
		if (pd->current_state == PE_UNKNOWN &&
				pd->current_dr == DR_NONE)
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (oplus_warp_get_fastchg_started()) {
		usbpd_err(&pd->dev, "%s: Warp fast charge started, return!\n", __func__);
		ret = size;
		goto out;
	}
	if (pd->current_voltage == 9000000 &&
		...
		/* Since the requested voltage is same as current voltage, we do nothing */
		ret = size;
		goto out;
	}
#endif
