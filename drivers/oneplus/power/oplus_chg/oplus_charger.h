...
#ifdef OEM_TARGET_PRODUCT_EBBA
	int pd_input_current_led_ma_high;
	int qc_input_current_led_ma_high;
	int pd_input_current_led_ma_warm;
	int qc_input_current_led_ma_warm;
	int pd_temp_warm_fastchg_current_ma_led_on;
	int qc_temp_warm_fastchg_current_ma_led_on;
#endif
};
...
/*	struct battery_data battery_main	*/
	...
	int pd_chging;
#ifdef OEM_TARGET_PRODUCT_EBBA
	int chg_protect_status_flag;
	int ship_mode_config;
	int qc_chging;
	bool led_on_actual;
	unsigned long led_st_debounce_expire;
#endif
};
...
int __weak get_eng_version(void) {return 0;};
bool __weak qpnp_is_charger_reboot(void) {return false;};
