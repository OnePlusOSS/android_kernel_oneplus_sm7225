...
static int rt5125_i2c_block_read(struct oplus_warp_chip *chip, u8 cmd, u8 * data, s32 len)
{
	u8 crc;
	int retry = 0, ret;
...
static int rt5125_i2c_block_write(struct oplus_warp_chip *chip, u8 cmd, const u8 * data, s32 len)
{
	int retry = 0, ret;
...
static int rt5125_fw_update(struct oplus_warp_chip *chip)
{
...
static bool rt5125_fw_update_check(struct oplus_warp_chip *chip)
{
	const u8 *data = chip->firmware_data;
	s32 len = chip->fw_data_count;
...
/******************************************************************/
static int rt5125_get_fw_verion_from_ic(struct oplus_warp_chip *chip)
{
...
static int rt5125_fw_check_then_recover(struct oplus_warp_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
	int ret = 0;
...
void rt5125_set_reset_active(struct oplus_warp_chip *chip)
{
	the_bat.uv_bat = 0;
	the_bat.current_bat = 0;
	opchg_set_reset_active(chip);
...
void rt5125_set_reset_sleep(struct oplus_warp_chip *chip)
{
	the_bat.uv_bat = 0;
	the_bat.current_bat = 0;
	opchg_set_reset_sleep(chip);
...
struct oplus_warp_operations oplus_rt5125_ops = {
...
//	ret = register_device_proc("warp", version, manufacture);
	ret = push_component_info(FAST_CHARGE, version, manufacture);
...
static int rt5125_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
	/*      if (get_warp_mcu_type(chip) != OPLUS_WARP_ASIC_HWID_RT5125) {
	...
	if (chip->warp_fw_type == WARP_FW_TYPE_RT5125_4400_WARP_FFC_15C) {
		chip->firmware_data = rt5125_fw;
		chip->fw_data_count = sizeof(rt5125_fw);
		chip->fw_data_version = rt5125_fw[chip->fw_data_count - 4];
	}
...
MODULE_DESCRIPTION("Driver for oplus warp rt5125 fast mcu");
