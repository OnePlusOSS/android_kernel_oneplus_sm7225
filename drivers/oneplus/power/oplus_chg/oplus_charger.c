		...
		enum power_supply_property psp)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_OP_DISABLE_CHARGE:
			rc = 1;
			break;
		case POWER_SUPPLY_PROP_PPT_MODE:
			rc = 1;
			break;
#endif
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
#ifdef OEM_TARGET_PRODUCT_EBBA
		case POWER_SUPPLY_PROP_OP_DISABLE_CHARGE:
			charger_xlog_printk(CHG_LOG_CRTI, "set op_disable_charge = [%d].\n", val->intval);
			if (val->intval == 1) {
				if (chip->unwakelock_chg == 1) {
					ret = -EINVAL;
					charger_xlog_printk(CHG_LOG_CRTI,
							"op_disable_charge:unwakelock testing , this test not allowed.\n");
				} else {
				#ifdef OEM_TARGET_PRODUCT_EBBA
		...
		case POWER_SUPPLY_PROP_PPT_MODE:
			if (val->intval == 1)
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 1000000);
			else
				vote(chg->fcc_votable, BATT_PROFILE_VOTER, true, 3000000);
			g_charger_chip->ppt_mode = val->intval;
			break;
#endif
		default:
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
			...
			if (!oplus_is_power_off_charging(NULL) &&
				...
				chip->chg_ops->oplus_usbtemp_monitor_condition() == true) {
				if (val->intval != POWER_SUPPLY_STATUS_FULL)
					val->intval = POWER_SUPPLY_STATUS_CHARGING;
				pr_debug("pd update charging first.\n");
			}
#endif
			break;
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			if (chip->pmic_spmi.smb5_chip->chg.fake_capacity >= 0) {
				val->intval = chip->pmic_spmi.smb5_chip->chg.fake_capacity;
				break;
			}
		...
		case POWER_SUPPLY_PROP_TEMP:
			if (chip->pmic_spmi.smb5_chip->chg.use_fake_temp) {
				val->intval = chip->pmic_spmi.smb5_chip->chg.fake_temp;
				break;
			}
			if (oplus_warp_get_fastchg_started() == true) {
		...
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			if (oplus_warp_get_fastchg_started() == true && (chip->vbatt_num == 2 || is_warp_support_single_batt_swarp() == true)
			&& oplus_warp_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WARP) {

				switch (oplus_warp_get_fast_chg_type()) {
				case  CHARGER_SUBTYPE_FASTCHG_WARP:
				case  WARP_ADAPTER_1:
				case  WARP_ADAPTER_2:
					val->intval = chip->charger_volt;
					break;
				case  SWARP_ADAPTER_1:
					val->intval = 10000;
					break;
				default:
					val->intval = 10000;
		...
		case POWER_SUPPLY_PROP_CHG_PROTECT_STATUS:
			...
			val->intval = chip->chg_protect_status_flag;
			break;
		case POWER_SUPPLY_PROP_PPT_MODE:
			val->intval = g_charger_chip->ppt_mode;
			if (val->intval)
				pr_err("ppt_mode = %d\n", val->intval);
			break;
#endif
		...
		const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		if ((is_warp_support_single_batt_swarp() || is_platform_fuel_gauge())
			&& mcu_status == 1) {
...
static ssize_t fastcharge_fail_count_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	char page[256] = {0};
	int len = 0;
	int fastcharge_fail_count = warp_get_fastcharge_fail_count();
...
static ssize_t proc_enhance_dash_read(struct file *file,
			char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	ret = snprintf(page, 4, "%d", chg_type);
	ret = simple_read_from_buffer(user_buf,
			count, ppos, page, strlen(page));
	return ret;
}

static ssize_t proc_enhance_dash_write(struct file *file,
			const char __user *buffer, size_t count, loff_t *ppos)
{
	pr_info("enhance_dash not supported write");
	return count;
}

static const struct file_operations proc_enhance_dash_operations = {
	.read = proc_enhance_dash_read,
	.write = proc_enhance_dash_write,
};
...
static int init_charger_proc(struct oplus_chg_chip *chip)
{
	...
	prEntry_tmp = proc_create_data("enhance_dash", 0664, NULL,
						&proc_enhance_dash_operations, chip);
	if (!prEntry_tmp) {
		ret = -1;
		chg_debug("Failed to register enhance_dash node\n");
	}
		...
		char __user *buff, size_t count, loff_t *off)
{
	...
	if (is_warp_support_single_batt_swarp() == true) {
		...
		case WARP_ADAPTER_1:
		case WARP_ADAPTER_2:
			swarp_is_control_by_warp = true;
			break;
		case SWARP_ADAPTER_1:
			swarp_is_control_by_warp = false;
			break;
		default:
			swarp_is_control_by_warp = true;
			break;
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,pd_input_current_led_ma_high",
		&chip->limits.pd_input_current_led_ma_high);
	if (rc) {
		chip->limits.pd_input_current_led_ma_high = chip->limits.input_current_led_ma;
	}

	rc = of_property_read_u32(node, "qcom,qc_input_current_led_ma_high",
		&chip->limits.qc_input_current_led_ma_high);
	if (rc) {
		chip->limits.qc_input_current_led_ma_high = chip->limits.input_current_led_ma;
	}

	rc = of_property_read_u32(node, "qcom,pd_input_current_led_ma_warm",
		&chip->limits.pd_input_current_led_ma_warm);
	if (rc) {
		chip->limits.pd_input_current_led_ma_warm = chip->limits.input_current_led_ma;
	}

	rc = of_property_read_u32(node, "qcom,qc_input_current_led_ma_warm",
		&chip->limits.qc_input_current_led_ma_warm);
	if (rc) {
		chip->limits.qc_input_current_led_ma_warm = chip->limits.input_current_led_ma;
	}

	rc = of_property_read_u32(node, "qcom,pd_temp_warm_fastchg_current_ma_led_on",
		&chip->limits.pd_temp_warm_fastchg_current_ma_led_on);
	if (rc) {
		chip->limits.pd_temp_warm_fastchg_current_ma_led_on = chip->limits.temp_warm_fastchg_current_ma;
	}

	rc = of_property_read_u32(node, "qcom,qc_temp_warm_fastchg_current_ma_led_on",
		&chip->limits.qc_temp_warm_fastchg_current_ma_led_on);
	if (rc) {
		chip->limits.qc_temp_warm_fastchg_current_ma_led_on = chip->limits.temp_warm_fastchg_current_ma;
	}

	charger_xlog_printk(CHG_LOG_CRTI,
		"pd_input_current_led_ma_high = %d, \
		qc_input_current_led_ma_high = %d, \
		pd_input_current_led_ma_warm = %d, \
		qc_input_current_led_ma_warm = %d, \
		pd_temp_warm_fastchg_current_ma_led_on = %d, \
		qc_temp_warm_fastchg_current_ma_led_on = %d\n",
		chip->limits.pd_input_current_led_ma_high,
		chip->limits.qc_input_current_led_ma_high,
		chip->limits.pd_input_current_led_ma_warm,
		chip->limits.qc_input_current_led_ma_warm,
...
#ifdef OEM_TARGET_PRODUCT_EBBA
			if (chip->pd_chging)
				charging_current = chip->limits.pd_temp_warm_fastchg_current_ma_led_on;
			else if (chip->qc_chging)
				charging_current = chip->limits.qc_temp_warm_fastchg_current_ma_led_on;
#endif
		}
...
#ifdef OEM_TARGET_PRODUCT_EBBA
				if (chip->qc_chging == true) {
					current_limit = (current_limit > chip->limits.qc_input_current_led_ma_high) ?
						chip->limits.qc_input_current_led_ma_high :
						current_limit;
				} else if (chip->pd_chging == true) {
					current_limit = (current_limit > chip->limits.pd_input_current_led_ma_high) ?
						chip->limits.pd_input_current_led_ma_high :
						current_limit;
				}
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
				if (chip->qc_chging == true) {
					current_limit = (current_limit > chip->limits.qc_input_current_led_ma_warm)
						? chip->limits.qc_input_current_led_ma_warm
						: current_limit;
				} else if (chip->pd_chging == true) {
					current_limit = (current_limit > chip->limits.pd_input_current_led_ma_warm)
						? chip->limits.pd_input_current_led_ma_warm
						: current_limit;
				}
#endif
...
static void battery_temp_anti_shake_handle(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__HIGH_TEMP) {										/*>50C*/
		...
		chip->limits.hot_bat_decidegc = chip->anti_shake_bound.hot_bound;						/*50C*/
		...
		chip->limits.warm_bat_decidegc = chip->anti_shake_bound.warm_bound;
		chip->limits.hot_bat_decidegc = chip->anti_shake_bound.hot_bound;
	} else if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP) {								/*45C~50C*/
		chip->limits.cold_bat_decidegc = chip->anti_shake_bound.cold_bound;
		chip->limits.little_cold_bat_decidegc = chip->anti_shake_bound.little_cold_bound;
		chip->limits.cool_bat_decidegc = chip->anti_shake_bound.cool_bound;
		chip->limits.little_cool_bat_decidegc = chip->anti_shake_bound.little_cool_bound;
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.hot_bat_decidegc || batt_temp < chip->limits.cold_bat_decidegc) {
...
#ifdef OEM_TARGET_PRODUCT_EBBA
				chip->chg_protect_status_flag = PROTECT_BATT_MISSING;
#endif
			} else if (batt_temp > chip->limits.hot_bat_decidegc) {
				tbatt_status = BATTERY_STATUS__HIGH_TEMP;
#ifdef OEM_TARGET_PRODUCT_EBBA
				chip->chg_protect_status_flag = PROTECT_BATT_TEMP_REGION__HOT;
#endif
			} else {
				tbatt_status = BATTERY_STATUS__LOW_TEMP;
#ifdef OEM_TARGET_PRODUCT_EBBA
				chip->chg_protect_status_flag = PROTECT_BATT_TEMP_REGION_COLD;
#endif
			}
		}
	} else {
		...
		if (batt_temp >= chip->limits.warm_bat_decidegc) {						/*45C*/
			...
			chip->chg_protect_status_flag = PROTECT_BATT_TEMP_REGION_WARM;
			...
			chip->chg_protect_status_flag = PROTECT_BATT_TEMP_REGION_LITTLE_COLD;
		} else {
			tbatt_status = BATTERY_STATUS__COLD_TEMP;
			chip->chg_protect_status_flag = PROTECT_BATT_TEMP_REGION_COLD;
		}
...
#ifdef OEM_TARGET_PRODUCT_EBBA
			chip->chg_protect_status_flag = PROTECT_CHG_OVP;
#endif
		}
...
#ifdef OEM_TARGET_PRODUCT_EBBA
			chip->chg_protect_status_flag = PROTECT_BATT_OVP;
#endif
		}
	} else {
		vbat_counts = 0;
...
#ifdef OEM_TARGET_PRODUCT_EBBA
		chip->chg_protect_status_flag = PROTECT_CHG_OVERTIME;
#endif
		return false;
	} else {
		chip->chging_over_time = false;
...
#ifdef CONFIG_FB
#if defined(OEM_TARGET_PRODUCT_EBBA)
...
void oplus_chg_lcd_check_status(struct oplus_chg_chip *chip)
{
	if (!chip)
		return;
	if (time_is_before_jiffies(chip->led_st_debounce_expire))
		chip->led_on = chip->led_on_actual;
	pr_info("[%s]led_on=%d, led_on_actual=%d\n", __func__, chip->led_on, chip->led_on_actual);
	return;
}
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (oplus_warp_get_fastchg_started() == true) {
		if (oplus_plat_gauge_is_support() && chip->mmi_chg != 0
				&& (mcu_status == 1 && oplus_warp_get_reset_gpio_status() == 1)) {
...
{
#ifdef OEM_TARGET_PRODUCT_EBBA
	if (true == oplus_chg_check_vchg_is_good(chip) &&
		 true == oplus_chg_check_vbatt_is_good(chip) &&
		 true == oplus_chg_check_time_is_good(chip))
		chip->chg_protect_status_flag = PROTECT_BATT_TEMP_REGION_COOL;
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
#define MAX_UPDATE_DELAY 144 // 12 MINUTES = 144 * 5S
#endif
static void battery_update(struct oplus_chg_chip *chip)
{
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	if (update_cnt < MAX_UPDATE_DELAY) {
		update_cnt += 1;
		need_update = 0;
	} else {
		need_update = 1;
	...
	if (rc < 0) {
		pr_err("Failed to get battery temp\n");
		//return;
	}

	if (chip->charger_exist || charger_exist_pre || pval.intval >= 450) {
		// Need update
		update_cnt = 0;
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (oplus_warp_get_fastchg_started() == false
			...
			if (chip->pd_swarp) {
				msleep(200);
				charger_xlog_printk(CHG_LOG_CRTI, "pd_swarp charger, delay 200ms to avoid interference with VCONN_Swap!\n");
			}
#endif
		...
		if (!oplus_warp_get_fastchg_started()
			...
			mcu_update = false;
			if ((is_warp_support_single_batt_swarp() || is_platform_fuel_gauge())
				&& oplus_warp_get_reset_gpio_status() == 0) {
				oplus_warp_set_ap_clk_high();
				oplus_warp_reset_mcu();
				mcu_status = 1;
			}
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	...
	if(chip->vbatt_num == 1){
		...
		charger_xlog_printk(CHG_LOG_CRTI,
			...
			chip->charging_state, chip->total_time, chip->hmac,
			...
			chip->led_on, chip->calling_on, chip->camera_on);
	#else
		charger_xlog_printk(CHG_LOG_CRTI,
			" CHGR[ %d / %d / %d / %d / %d ], "
			"BAT[ %d / %d / %d / %d / %d / %d ], "
...
static void oplus_chg_update_work(struct work_struct *work)
{
	...
	if (register_drm <= 2) {
		if (lcd_active_panel) {
			...
			if (rc) {
				pr_err("CHG Unable to register drm_panel_notifier: %d\n", rc);
			} else {
				pr_err("CHG register notifier drm_panel_notifier success!\n");
			}
		} else {
			pr_err("CHG Ooops!!! lcd_active_panel is null, unable to register drm_panel_notifier!\n");
		}
		register_drm++;
	}
#endif
	...
	if (chip->charger_exist) {
#ifdef OEM_TARGET_PRODUCT_EBBA
		chip->camera_on = *chg->video_call_on;
		oplus_chg_lcd_check_status(chip);
		oplus_warp_set_lcd(g_charger_chip->led_on, g_charger_chip->calling_on);
#endif
...
bool oplus_chg_get_pd_swarp(void)
{
	if (!g_charger_chip)
		return false;
	else
		return g_charger_chip->pd_swarp;
}
#endif
...
int oplus_chg_get_skin_temperature(void)
{
	int rc = 0;
	int skin_temp = 0;
	...
	if (!g_charger_chip) {
		...
		chg = &g_charger_chip->pmic_spmi.smb5_chip->chg;
		rc = smblib_read_iio_channel(chg, chg->iio.skin_therm_chan,
					DIV_FACTOR_DECIDEGC, &skin_temp);
		if (rc < 0) {
			chg_err("Couldn't read SKIN THERM channel, rc=%d\n", rc);
			return rc;
		}
		return skin_temp;
	}
...
int oplus_chg_show_warp_logo_ornot(void)
{
	...
	if (g_charger_chip->wireless_support
		...
		return 1;
	//chg_err("oplus_warp_get_fastchg_started=%d,oplus_warp_get_fast_chg_type=%d\n",(int)oplus_warp_get_fastchg_started(),(int)oplus_warp_get_fast_chg_type());
...
#ifdef OEM_TARGET_PRODUCT_EBBA
		//charger_xlog_printk(CHG_LOG_CRTI, "charging override by skin_therm\n");
#else
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (chip->smart_charge_version == 1) {
		...
		chip->cool_down =
				oplus_warp_get_warp_multistep_adjust_current_support() ?
						val : 1;
		if (val > 6) {
			chip->limits.input_current_charger_ma =
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_chg_override_by_shell_temp(batt_temp)) {
		shell_temp = oplus_chg_get_skin_temperature();
	...
	charger_xlog_printk(CHG_LOG_CRTI, "batt_temp=%d,skin_therm=%d,chging_temp=%d\n",
