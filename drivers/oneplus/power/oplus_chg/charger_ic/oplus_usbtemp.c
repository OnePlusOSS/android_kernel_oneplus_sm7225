...
int oplus_usbtemp_dischg_action(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		if (qpnp_is_power_off_charging() == true) {
			chg_err("[%s]: in poweroff charging, device will boot up\n", __func__);
			chg->disconnect_vbus = 1;
			rc = pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.dischg_enable);
			msleep(20);
			machine_restart(NULL);
		} else {
			chg->disconnect_vbus = 1;
			chg_err("[%s]: set vbus down\n", __func__);
			rc = pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.dischg_enable);
			kobject_uevent_env(&chg->dev->kobj, KOBJ_CHANGE, detected);
			chg_err("[%s]: usb connecter hot,Vbus disconnected! Sent uevent %s\n",
				 detected[0], __func__);

			chg_err("wait 4h and will start recover_func!\n");
			alarm_start_relative(&chip->usbotp_recover_timer, ktime_set(14400, 0));
		}
#else
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	if (value) {
		__pm_stay_awake(g_oplus_chip->usbtemp_wakelock);
	} else {
		__pm_relax(g_oplus_chip->usbtemp_wakelock);
	}
#else
	if(value) {
...
#ifdef OEM_TARGET_PRODUCT_EBBA
enum alarmtimer_restart recover_charge_hrtimer_func(
	struct alarm *alarm, ktime_t now)
{
	if (g_oplus_chip == NULL) {
		chg_err("[%s]: get chip status latter\n", __func__);
		return ALARMTIMER_NORESTART;
	}

	chg_err("[%s]: recover work start.\n", __func__);
	oplus_set_usbtemp_wakelock(true);
	schedule_delayed_work(&g_oplus_chip->usbtemp_recover_work, msecs_to_jiffies(0));
	return ALARMTIMER_NORESTART;
}
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
EXPORT_SYMBOL(recover_charge_hrtimer_func);
#endif
